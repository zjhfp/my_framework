...
import com.pan.bbf.common.ReflectUtil;
...
public class PageInterceptor implements Interceptor {
	...
	private Logger log = Logger.getLogger(PageInterceptor.class);
	 ...
	 * @throws Throwable
	 */
	protected Object interceptExecutor(Invocation invocation) throws Throwable{
	...
	protected Pagination findPageObject(Object paramObj){
	 ...
	 * @throws Throwable
	 */
	protected Object interceptStatementHandler(Invocation invocation) throws Throwable{
		...
		// 判断参数里是否有page对象
		Pagination page = findPageObject(paramObj);
		...
		if (page != null) {
			...
			// 给当前的page参数对象设置总记录数
			if (page.getTotalRecords() < 0) { // 如果总数为负数表需要设置
	...
	private void setTotalRecord(Object obj, MappedStatement mappedStatement, Connection connection, Pagination page) {
		...
		try {
			...
			log.error("setTotalRecord error",e);
		} finally {
			try {
				if (rs != null)
					rs.close();
